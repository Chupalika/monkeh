...
class Koduck:
    ...
    async def send_message(self, receive_message: Optional[Union[discord.Message, discord.Interaction]] = None, channel: Optional[discord.abc.Messageable] = None, ignore_cd: bool = False, **kwargs):
        ...
        #apparently this returns None instead of a message object!!
        ...
        if receive_message is not None:
            ...
            elif isinstance(receive_message, discord.Interaction):
                the_message = await receive_message.original_response()
                if not the_message.flags.ephemeral:
                    user_last_outputs.append(receive_message)
        ...
        if type == "prefix":
            ...
            async def wrapper_function(interaction, **kwargs):
                ...
                if interaction.command.koduck.get_user_level(interaction.user.id) < interaction.command.command_tier:
                    return await koduck_instance.send_message(interaction, content=settings.message_restricted_access)
                try:
                    ...
                    traceback.print_exc()
                    await koduck_instance.send_message(interaction, content=settings.message_something_broke + "\n``{}``".format(error_message))
